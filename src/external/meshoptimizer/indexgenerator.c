...
size_t hash_sub(VertexHasher* vertexHasher, unsigned int index)
{
	return hashUpdate4(0, vertexHasher->vertices + index * vertexHasher->vertex_stride, vertexHasher->vertex_size);
}

bool VertexHasher_equal(VertexHasher* vertexHasher, unsigned int lhs, unsigned int rhs)
{
	return memcmp(vertexHasher->vertices + lhs * vertexHasher->vertex_stride, vertexHasher->vertices + rhs * vertexHasher->vertex_stride, vertexHasher->vertex_size) == 0;
}
...
size_t VertexStreamHasher_hash(VertexStreamHasher* vertexStreamHasher, unsigned int index)
{
	unsigned int h = 0;

	for (size_t i = 0; i < vertexStreamHasher->stream_count; ++i)
	{
		const meshopt_Stream* s = &vertexStreamHasher->streams[i];
...
bool VertexStreamHasher_equal(VertexStreamHasher* vertexStreamHasher, unsigned int lhs, unsigned int rhs)
{
	for (size_t i = 0; i < vertexStreamHasher->stream_count; ++i)
	{
		const meshopt_Stream* s = &vertexStreamHasher->streams[i];
...
//	return 0;
//}
size_t VertexHasher_hash(VertexHasher* vertexHasher, unsigned int index)
{
	return hashUpdate4(0, vertexHasher->vertices + index * vertexHasher->vertex_stride, vertexHasher->vertex_size);
}

static unsigned int* VertexHasher_hashLookup(unsigned int* table, size_t buckets, VertexHasher hash, const unsigned int key, const unsigned int empty)
{
	assert(buckets > 0);
	assert((buckets & (buckets - 1)) == 0);
...
static unsigned int* VertexStreamHasher_hashLookup(unsigned int* table, size_t buckets, VertexStreamHasher hash, const unsigned int key, const unsigned int empty)
{
	assert(buckets > 0);
	assert((buckets & (buckets - 1)) == 0);

	size_t hashmod = buckets - 1;
	size_t bucket = VertexStreamHasher_hash(&hash, key) & hashmod;
	...
	for (size_t probe = 0; probe <= hashmod; ++probe)
	{
		...
		if (VertexStreamHasher_equal(&hash, *item, key))
			return item;
...
size_t meshopt_generateVertexRemap(unsigned int* destination, const unsigned int* indices, size_t index_count, const void* vertices, size_t vertex_count, size_t vertex_size)
{
	...
	for (size_t i = 0; i < index_count; ++i)
	{
		...
		{
			unsigned int* entry = VertexHasher_hashLookup(table, table_size, hasher, index, ~0u);
...
size_t meshopt_generateVertexRemapMulti(unsigned int* destination, const unsigned int* indices, size_t index_count, size_t vertex_count, const struct meshopt_Stream* streams, size_t stream_count)
{
	...
	for (size_t i = 0; i < index_count; ++i)
	{
		...
		if (destination[index] == ~0u)
		{
			unsigned int* entry = VertexStreamHasher_hashLookup(table, table_size, hasher, index, ~0u);
